  ...
  return (
    <div>
      ...
      <div className=" pl-10 my-2 flex justify-between gap-[100px]">
          ...
          <div className="flex flex-col mt-[140px] max-w-[380px] gap-[30px] relative" data-aos="fade-right"
          ...
          <img src={currentTemplate.Arrow} alt="" className={`${currentIndex === 1 ? ' absolute bottom-[100%] left-[-20%]' : currentIndex === 2 ? ' absolute top-[-5%] left-[150%]' : currentIndex === 0 ? ' absolute top-[68%] left-[-30%]' : '' }  `} />
          </div>

          <div>
            <img src={currentTemplate.image} alt="" className=" relative mr-[-50px] h-[850px]" />
            <img src={currentTemplate.imageSrc} alt="" className=" absolute top-[40%] left-[42%]" />
            <button  onClick={handlePrevious} className={`${currentIndex < 1 ? 'hidden' : ''}`} > 
                 <MdOutlineKeyboardArrowLeft  size={40} className=" bg-white shadow-lg rounded-full text-[#191919] absolute top-[70%] left-[40.5%]"/>
               </button>
            <button onClick={handleNext} className={`${ currentIndex >= 2 ? 'hidden' : ''}`}> 
                 <MdOutlineKeyboardArrowRight  size={40} className=" bg-white shadow-lg rounded-full text-[#191919] absolute top-[70%] right-[24.5%]"/>
               </button>
          </div>
